...
public final class StreamService {
  ...
  public PlayerAccessRecord link(
      UUID playerId, String playerName, StreamProviderId providerId, String channel) {
    PlayerAccessRecord accessRecord =
        repository
            .getOrCreate(playerId, playerName)
            .withStreamLink(
                new StreamLink(providerId, linkNormalizer.normalize(providerId, channel)))
            .withVerificationStatus(
                VerificationStatus.unverified(clock.instant(), "Stream link changed."));
  ...
  public VerificationStatus verify(UUID playerId, String playerName) {
    ...
    if (link.isEmpty()) {
      VerificationStatus status =
          VerificationStatus.unverified(clock.instant(), "No stream link configured.");
    ...
    if (!result.available()) {
      return accessRecord
          ...
          .orElseGet(() -> VerificationStatus.unverified(clock.instant(), result.detail()));
    }
    VerificationStatus status = statusFromResult(result);
    if (repository.saveIfUnchanged(
        new PlayerAccessUpdate(accessRecord, accessRecord.withVerificationStatus(status)))) {
      return status;
    }
    return repository
        .getOrCreate(playerId, playerName)
        .verificationStatusOptional()
        .orElseGet(
            () -> VerificationStatus.unverified(clock.instant(), "Player stream state changed."));
  }
  ...
  public Map<UUID, VerificationStatus> verifyAll(Collection<StreamVerificationTarget> targets) {
    ...
    for (StreamVerificationTarget target : targets) {
      PlayerAccessRecord accessRecord =
          repository.getOrCreate(target.playerId(), target.playerName());
      expectedRecords.put(target.playerId(), accessRecord);
      accessRecord.streamLinkOptional().ifPresent(link -> links.put(link, link));
    }

    Map<StreamLink, VerificationResult> verificationResults =
        verificationProvider.verifyAll(links.keySet());
    ...
    for (StreamVerificationTarget target : targets) {
      ...
      if (link == null) {
        ...
        VerificationResult result =
            verificationResults.getOrDefault(
                link,
                VerificationResult.unavailable(
                    link.providerId(), "Verification provider returned no result."));
  ...
  public void manuallyVerify(UUID playerId, String playerName, String detail) {
    VerificationStatus status =
        VerificationStatus.live(StreamProviderId.MANUAL, clock.instant(), detail);
