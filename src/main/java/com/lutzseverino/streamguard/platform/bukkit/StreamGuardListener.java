...
public final class StreamGuardListener implements Listener {
  ...
  public void onInteract(PlayerInteractEvent event) {
    ...
    GuardedAction action =
        isContainerLike(event.getClickedBlock().getType())
  ...
  public void onInventoryOpen(InventoryOpenEvent event) {
    if (event.getPlayer() instanceof Player player) {
      GuardedAction action =
          event.getInventory().getType().name().contains("MERCHANT")
  ...
  public void onInteractEntity(PlayerInteractEntityEvent event) {
    GuardedAction action =
        event.getRightClicked() instanceof AbstractVillager
            ? GuardedAction.VILLAGER_TRADING
  ...
  private void cancelIfDenied(Player player, GuardedAction action, Cancellation cancellation) {
    AccessDecision decision =
        accessService.decide(
            player.getUniqueId(), player.getName(), action, hasPermissionBypass(player));
    if (decision.allowed()) {
      return;
    }
    cancellation.cancel();
    ...
    player.sendMessage(
        messages.renderLegacyDefault(
            messageKey(state, "interaction"), Map.of("action", action.configKey())));
  }
  ...
  private GateState gateState(Player player) {
    return accessService.gateState(
        player.getUniqueId(), player.getName(), hasPermissionBypass(player));
  }
        ...
        .runTaskLater(
            ...
            () -> {
              ...
              if (stateRules(current).kickOnJoin()) {
                player.kickPlayer(
                    messages.renderLegacyDefault(messageKey(current, "kick"), Map.of()));
              }
            },
            ticks);
  }
