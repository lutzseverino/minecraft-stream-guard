...
public record StreamGuardSettings(
  ...
  public static StreamGuardSettings load(SettingsReader reader) {
    ...
    return new StreamGuardSettings(
        ...
        new Enforcement(
            ...
            Duration.ofSeconds(
                Math.max(5, reader.integer("enforcement.recheck-interval-seconds", 60))),
            guardedActions,
  ...
  public record Language(String defaultLocale, String fallbackLocale) {}
  ...
  public record StateRules(
    ...
    private static StateRules load(SettingsReader reader, String path) {
  ...
  public record Verification(Cache cache, Duration maximumStatusAge) {
    private static Verification load(SettingsReader reader) {
      return new Verification(
          Cache.load(reader),
          Duration.ofSeconds(
              Math.max(30, reader.integer("verification.maximum-status-age-seconds", 180))));
    }
  }
  ...
  public record Cache(Duration liveTimeToLive, Duration offlineTimeToLive) {
    private static Cache load(SettingsReader reader) {
      return new Cache(
          ...
          Duration.ofSeconds(
              Math.max(0, reader.integer("verification.cache.offline-seconds", 120))));
    }
  }
  ...
  public record Onboarding(boolean enabled, ProviderPicker providerPicker, ChatInput chatInput) {
  ...
  public record ProviderPicker(
    ...
    private static ProviderPicker load(SettingsReader reader) {
      List<ProviderButton> providers =
          reader.keys("onboarding.provider-picker.providers").stream()
              .flatMap(
                  rawKey ->
                      StreamProviderId.parse(rawKey)
                          .map(
                              providerId ->
                                  ProviderButton.load(
                                      reader,
      ...
      return new ProviderPicker(
          reader.string(
              "onboarding.provider-picker.title", "<dark_gray>Choose stream provider</dark_gray>"),
          ...
          GuiItem.load(
              reader,
              "onboarding.provider-picker.filler",
          ...
          GuiItem.load(
              reader,
              "onboarding.provider-picker.cancel",
              "BARRIER",
              "<red>Cancel</red>",
              List.of(),
              22,
  ...
  public record ProviderButton(
      StreamProviderId providerId, boolean enabled, int slot, String inputHint, GuiItem item) {
    private static ProviderButton load(
        SettingsReader reader, String path, StreamProviderId providerId) {
      return new ProviderButton(
          ...
          GuiItem.load(
              ...
              path,
              "PAPER",
              "<aqua>" + providerId.displayName() + "</aqua>",
              List.of(),
              0,
              false));
    }
  ...
  public record ChatInput(
      int timeoutSeconds, int maxLength, String cancelKeyword, boolean verifyAfterLink) {
  ...
  public record Bypass(
      boolean opsBypassByDefault, boolean allowTemporaryBypass, int maxTemporaryBypassMinutes) {}
                ...
                providerId -> {
                  ...
                  providers.put(
                      ...
                      new ProviderSettings(
                          providerId, reader.bool(path + ".enabled", false), options));
                });
      }
  ...
  public record ProviderSettings(
      StreamProviderId providerId, boolean enabled, Map<String, String> options) {
